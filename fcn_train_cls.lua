...
function fcn.train(inputs_all)
    ...
    _, fs = optim.sgd(opfunc, params_FCN, optimState)
    batchNumber = batchNumber + 1
    cutorch.synchronize(); collectgarbage();
    print(('Epoch: [%d][%d/%d]\tTime %.3f DataTime %.3f, loss %.3f'):format(epoch, batchNumber, opt.epochSize, sampleTimer:time().real, dataLoadingTime, fs[1]))
    dataTimer:reset()
end
